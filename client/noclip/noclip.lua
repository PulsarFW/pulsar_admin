...
ADMIN.NoClip = {
    ...
    Stop = function(self)
        if NOCLIP_ACTIVE then
            ...
            if not plsr.State.flags.isAdmin or not plsr.State.flags.isGodmode then
...
RegisterNetEvent('Admin:Client:NoClipInfo', function()
    if NOCLIP_ACTIVE then
        local position = GetFreecamPosition()
        local rotation = GetFreecamRotation()
        plsr.Logger:Trace('NoClip', 'Rotation: '.. rotation)
        plsr.Logger:Trace('NoClip', 'Position: '.. position)
    end
end)